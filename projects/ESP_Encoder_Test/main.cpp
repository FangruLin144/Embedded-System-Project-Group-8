...
DigitalOut motorBipolar(PB_1);

QEI encoderLeft(PA_13, PA_14, NC, 256, QEI::X4_ENCODING);
QEI encoderRight(PA_15, PB_7, NC, 256, QEI::X4_ENCODING);

int main() {
    float pwm_period;

    motorBipolar.write(0);
    motorDirection.write(1);
    motorEnable.write(1);

    while(1) {
        pwm_period = 1.0f / 25000.0f; // Unit: s
        ...
        lcd.locate(0, 0);
        lcd.printf("Pulses is: %d              \n", encoderRight.getPulses());
        lcd.printf("Encoder status: %d         \n", encoderRight.getCurrentState());
        wait(0.1);
    }
}
